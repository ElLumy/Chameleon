...
(function() {
  ...
  async function initialize() {
    try {
      ...
      // Verificar que tenemos los datos de perfiles
      if (!window.CHAMELEON_PROFILES_DATA) {
        throw new Error('Profiles data not loaded');
      }
      
      // Generar perfil
      const generator = ProfileGenerator(window.CHAMELEON_PROFILES_DATA, ChameleonState.seed);
  ...
  function waitForDependencies() {
    return new Promise((resolve) => {
      ...
      const check = () => {
        ...
        if (window.Math.seedrandom && window.CHAMELEON_PROFILES_DATA) {
          resolve();
